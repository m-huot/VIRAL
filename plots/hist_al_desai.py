...
def main(EMBED):
    ...
    df, train_x, train_y, targets = load_and_preprocess_data(EMBED, df="desai")

    dataset = Dataset_perso(train_x, train_y)

    FOLDER = f"../script_results/hist_al_desai_{EMBED}"
    ...
    get_hist_desai(
        NB_POINTS, NB_ROUNDS, NB_RUNS, NB_0, site_rbd_list, dataset, FOLDER, df, EMBED
    )
